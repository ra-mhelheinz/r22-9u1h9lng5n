...
<div id="content">
  <div id="content-header">
    @php
      $_bc = ['<a href="#">Master File</a>', '<a href="#">Inventory</a>', '<a href="" class="current">Generic</a>'];
    @endphp
          ...
          <div class="widget-content nopadding">
            ...
            <table id="" class="table table-bordered data-table">
              ...
              <tbody>
                  @foreach($data as $d)
                  <tr class="gradeX">
                    <td><center>{{$d->gen_code}}</center></td>
                    <td class="center"><center>{{$d->gen_name}}</center></td>
                    <td class="center">
                      <center>
                        <a href="#Update" pd-popup-open="small" class="btn btn-info" type="button" onclick="EditMode('{{$d->gen_code}}', '{{$d->gen_name}}');"><i class="icon-pencil"></i></a>
                        <a href="#" pd-popup-open="small" class="btn btn-danger" type="button" onclick="DeleteMode('{{$d->gen_code}}', '{{$d->gen_name}}')"><i class="icon-remove"></i></a>
                      </center>
                    </td>
                  </tr>
                  @endforeach
          ...
          <div class="row-fluid">
            <div class="col-lg-10"> 
              ...
              <div class="DeleteMode control-group col-lg-12" style="display: none">
                <div class="controls">
                  <label for="">Are you sure you want to delete <span style="color:red;font-weight: bold" id="DeleteName"></span> from Generic List ?</label>
                </div>
              </div>
            </div>
          </div>
   ...
   function emptyAdd()
  {
    ...
    $('#EditMode').hide();
    $('#AddForm').attr('action', '{{url('master-file/inventory/generic')}}');
    $('.EditMode').hide();
    $('.AddMode').show();
    $('.DeleteMode').hide();
    status('(Add)');
    ...
    $('#AddForm').attr('action', '{{url('master-file/inventory/generic/update')}}');
    $('#EditMode').show();
    $('.AddMode').hide();
    $('.EditMode').show();
    $('.DeleteMode').hide();
  ...
  function DeleteMode(code, desc)
  {
    ...
    $('#AddForm').attr('action', '{{url('master-file/inventory/generic/delete')}}');
    $('.AddMode').hide();
    $('.EditMode').hide();
    $('.DeleteMode').show();
    status('(Delete)');
